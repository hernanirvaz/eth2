...
if pig; then lgm "WAN is UP. No action needed.";exit 0;fi
...
lgm "DHCP renewal failed. Attempting to bounce WAN interface $WAN..."
/sbin/ifconfig "$WAN" down;sleep 5;/sbin/ifconfig "$WAN" up;sleep 15
if pig; then lgm "WAN recovered after WAN bounce.  ";exit 0;fi

lgm "Interface bounce failed. Performing system reboot as last resort."
/sbin/reboot
